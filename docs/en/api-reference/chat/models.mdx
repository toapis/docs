---
title: "Supported Text Models"
sidebarTitle: "Model List"
description: "Text model list supporting both Chat Completions and Responses API formats"
---

The platform provides two API formats. Use the `model` parameter to switch models:

| API Format | Endpoint | Description |
|----------|------|------|
| **Chat Completions** (default) | `POST /v1/chat/completions` | Classic chat API, compatible with OpenAI SDKs. [View docs](./chat) |
| **Responses** (new) | `POST /v1/responses` | Newer OpenAI agentic API with function calling, built-in tools, and server-side multi-turn context. [View docs](./responses) |

<Tip>
Models marked as <code>Responses Only</code> only support the Responses API and do not support Chat Completions. Other models are called through Chat Completions by default.
</Tip>
...
| Model ID | Description | Vision | Thinking | API Format |
|---------|------|:----:|:--------:|:--------:|
| `gpt-5-pro-official` | Official GPT-5 Pro, OpenAI's strongest flagship reasoning model | ✓ | ✓ | Responses Only |
| `gpt-5.2-official` | Official GPT-5.2 with enhanced multimodal capabilities | ✓ | ✓ | All |
| `gpt-5.3-codex-official` | Official GPT-5.3 Codex, latest code-optimized model with function calling and built-in tools | ✓ | ✓ | Responses Only |
...
| `gpt-5.6-sol` | GPT-5.6 flagship tier for complex reasoning, high-value coding, and long-context work | ✓ | ✓ |
| `gpt-5.6-terra` | GPT-5.6 balanced tier for general production traffic and agent workflows | ✓ | ✓ |
| `gpt-5.6-luna` | GPT-5.6 high-throughput tier for classification, extraction, routing, and batch jobs | ✓ | ✓ |
