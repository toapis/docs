...
<Note>
  **Querying Task Results**
  ...
  Video generation is an asynchronous task. After submission, a `task_id` is returned. Use the [Get Video Task Status](../../tasks/video-status) interface to query generation progress and results.
</Note>
