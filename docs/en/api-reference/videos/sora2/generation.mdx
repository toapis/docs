---
title: "Sora2 VVIP Video Generation"
sidebarTitle: "Sora2 VVIP"
api: "POST https://toapis.com/v1/videos/generations"
description: "Generate high-quality videos using the sora-2-vvip model"
---

- `sora-2-vvip` video generation model
...
<ParamField body="model" type="string" default="sora-2-vvip" required>
  ...
  Supported model: `sora-2-vvip` - VVIP version with fixed duration tiers
...
<ParamField body="prompt" type="string" required>
  ...
  **💡 @Character Reference Feature:**
  ...
  - The character's `username` can be obtained via the [Query Character API](./get-personas)
...
<ParamField body="duration" type="integer">
  ...
  Supports only `4`, `8`, or `12` seconds
...
```bash cURL (sora-2-vvip fixed duration)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
...
```bash cURL (With Character Reference)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
...
```javascript JavaScript (with metadata)
