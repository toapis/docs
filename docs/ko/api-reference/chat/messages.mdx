...
description: "Chat with Claude models using the native Anthropic Messages API format"
---
...
<Note>
  If you are already using the OpenAI SDK, use the [OpenAI-compatible endpoint](./chat) instead.
    ...
    <ParamField body="content" type="string | object[]" required>
      ...
      <Expandable title="Content block">
        ...
        <ParamField body="text" type="string">
          Text content. 필수 when `type` is `text`.
        </ParamField>
        <ParamField body="source" type="object">
          Image source. 필수 when `type` is `image`.
          ...
          <Expandable title="source">
            ...
            <ParamField body="data" type="string">
              Base64-encoded image data. 필수 when `type` is `base64`.
            </ParamField>
            <ParamField body="url" type="string">
              Image URL. 필수 when `type` is `url`.
            </ParamField>
          </Expandable>
        </ParamField>
      </Expandable>
...
```python Python (with system prompt)
import anthropic
