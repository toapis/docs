...
<Warning>
Please provide publicly accessible image URLs. Do not pass base64 image data directly in `image_urls` or `metadata`; upload local images first using the [Upload 이미지 API](../../uploads/images) to get URLs.
</Warning>
...
<ParamField body="model" type="string" required>
  모델 name, fixed as `MiniMax-Hailuo-02`.
</ParamField>

<ParamField body="prompt" type="string">
  Video content description, max **2000 characters**.

  - **필수** for text-to-video mode
  - 선택 for image-to-video and first-last-frame modes (can describe desired motion)
...
| Mode | image_urls | metadata.last_frame_image | 설명 |
|------|:----------:|:-------------------------:|-------------|
