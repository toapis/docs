...
description: "Generate high-quality videos using Alibaba Cloud Wan2.6 model — supports text-to-video, image-to-video and reference-video"
---
...
<Warning>
**Important Change**: For better performance and cost control, we no longer support passing base64 image data directly in `image_urls`. Please use the [Upload 画像API](../../uploads/images) first to upload images and get URLs, then call this endpoint.
</Warning>
...
<ParamField body="prompt" type="string" required>
  ...
  必須 for text-to-video mode; optional for image-to-video and reference video modes (describe expected motion or style)
...
<ParamField body="image_urls" type="string[]">
  ...
  **⚠️ URL format only (base64 no longer supported)**
  ...
  - You can use the [Upload 画像API](../../uploads/images) to upload local images and get URLs
...
| Aspect Ratio | 説明 | 720p Size | 1080p Size |
|---|---|---|---|
...
### Scenario 4: Text-to-Video (Full パラメータ)
...
<Note>
  **Querying Task Results**
  ...
  Video generation is an asynchronous task. After submission, a `task_id` is returned. Use the [Get Video Task ステータス](../../tasks/video-status) interface to query generation progress and results.
</Note>
