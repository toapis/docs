---
title: "Gemini Omni Flash 動画生成"
sidebarTitle: "Gemini Omni Flash"
api: "POST https://toapis.com/v1/videos/generations"
description: "非公式の Gemini Omni Flash チャネルでテキストまたは最大 3 枚の参照画像から動画を生成"
---
...
<Warning>
Upload local images with the [Upload 画像API](../../uploads/images) first, then pass the returned URLs in `image_urls`. Do not pass base64 image data directly.
</Warning>
...
<ParamField body="model" type="string" default="gemini-omni-flash" required>
  モデル name. Use `gemini-omni-flash`; some channels expose the `gemini_omni_flash` alias.
</ParamField>
