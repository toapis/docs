---
title: "Doubao SeeDance 動画生成"
sidebarTitle: "doubao-seedance"
api: "POST https://toapis.com/v1/videos/generations"
description: "Generate videos using ByteDance Doubao SeeDance model"
---
...
<RequestExample>
```bash cURL (Quick Preview Landscape Video)
curl --request POST \
...
```bash cURL (High-Quality Portrait Short Video)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
...
```bash cURL (Dynamic Transition Effect - First/Last Frame)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
