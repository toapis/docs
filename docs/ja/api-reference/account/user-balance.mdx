---
title: "ユーザー残高の照会"
sidebarTitle: "ユーザー残高の照会"
api: "GET https://toapis.com/v1/user/balance"
description: "Query the remaining and used quota of the current user account"
---
...
def get_user_balance():
    ...
    if data.get('success'):
        if data.get('unlimited_quota'):
            ...
            print(f"User remaining credits: {data['remain_credits']}")
        print(f"User used: {data['used_balance']}")
        print(f"User used credits: {data['used_credits']}")
    else:
...
async function getUserBalance() {
  ...
  if (data.success) {
    if (data.unlimited_quota) {
      ...
      console.log(`User remaining credits: ${data.remain_credits}`);
    }
    console.log(`User used: ${data.used_balance}`);
    console.log(`User used credits: ${data.used_credits}`);
  } else {
