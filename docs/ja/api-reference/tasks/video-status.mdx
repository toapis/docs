---
title: "Get Video Task ステータス"
sidebarTitle: "Get Video Task ステータス"
    ...
    <ResponseField name="data" type="array">
      ...
      <Expandable title="Array item">
        ...
        <ResponseField name="last_frame_url" type="string">
          生成動画の最終フレーム画像 URL。`return_last_frame: true` で作成し、モデルがフレームを返した場合のみ含まれます。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
<Note>
  ...
  1. **ToAPIs Task Webhook**: [Webhook](/docs/ja/api-reference/webhooks/task-webhooks) を主経路、照会をフォールバックにします
...
[非同期タスクのレート制限](/docs/ja/api-reference/rate-limits/async-tasks)を参照してください。バッチ照会は最大100件です。
