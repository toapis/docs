...
OpenAI 兼容的聊天 API，支持 GPT-5、Claude Sonnet 4.5、Gemini 2.0。切换 Base URL，无需修改代码。](./api-reference/chat/chat)
...
支持流式传输和低延迟](./api-reference/chat/chat)
### [​](#图像生成-api)图像生成 API

支持 GPT-4o Image、Gemini 2.5 Flash Image-preview 模型。OpenAI 风格 API，无缝集成。
[## 图像生成 API

文本到图像生成](./api-reference/images/gpt-4o/generation)
### [​](#视频生成-api)视频生成 API

支持 OpenAI Sora2、Google VEO3 模型。异步任务管理，支持状态追踪和 Webhook。
[## 视频生成 API

异步任务管理，支持 Webhook](./api-reference/videos/veo3/generation)
## [​](#平台优势)平台优势
...
监控端点健康和性能。追踪图像和视频生成的异步任务进度，支持 Webhook 通知。[查看视频状态 →](./api-reference/tasks/video-status)
## [​](#sdk-与代码示例)SDK 与代码示例
...
通过 `npm install openai` 安装。配置 baseURL 参数为 ToAPIs 端点，保留现有集成。
复制
```shiki
...
OpenAI 兼容的 Java 客户端。简单配置更改即可访问所有 ToAPIs 模型和供应商。
...
注册 ToAPIs 并从[控制台](https://toapis.com/console/token)获取 API Key。2

更新 Base URL

将 OpenAI Base URL 改为 ToAPIs 端点。现有 SDK 集成无需修改即可工作。3
...
ToAPIs 提供透明的按 Token 定价，比官方供应商更实惠。按需付费无需订阅，企业用量享受批量折扣，无隐藏费用。

延迟和性能如何？

通过全球 CDN 加速和智能多供应商路由保证低延迟。全球边缘节点确保最优响应时间。实时状态监控让您追踪性能。[通用对话接口](./api-reference/chat/chat)