...
description: "通过 API 创建新的 API 令牌"
---
...
<ParamField body="remain_credits" type="float" default="0">
  令牌剩余积分。如果传入该字段，会覆盖 `remain_quota`

  换算关系：`200` 积分 = $1 USD = `500000` 内部额度

  示例：`200`（相当于 $1）
</ParamField>

<ParamField body="daily_credits" type="float" default="0">
  令牌每日限额积分。如果传入该字段，会自动换算为 `daily_quota`

  示例：`100`（相当于每日 $0.5 限额）
</ParamField>

<ParamField body="monthly_credits" type="float" default="0">
  令牌每月限额积分。如果传入该字段，会自动换算为 `monthly_quota`
