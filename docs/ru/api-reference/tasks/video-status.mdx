...
sidebarTitle: "Получить статус видео"
api: "GET https://toapis.com/v1/videos/generations/{task_id}"
description: "Запрос статуса и результатов задачи генерации видео"
---
...
print(f"URL видео: {result['result']['data'][0]['url']}")
```
...
waitForVideo(taskId).then(result => {
  console.log('URL видео:', result.result.data[0].url);
  console.log('Формат:', result.result.data[0].format);
});
```
...
func main() {
    ...
    for i := 0; i < 60; i++ {
        ...
        if status == "completed" {
            ...
            fmt.Println("URL видео:", videoData["url"])
            break
        }
...
<ResponseField name="status" type="string">
  Статус задачи
  - `queued` - Отправлена, ожидает обработки
...
<ResponseField name="result" type="object">
  Результат задачи (возвращается только при успехе)
  
  <Expandable title="Свойства">
    <ResponseField name="type" type="string">
      Тип результата, всегда `video`
    </ResponseField>
    <ResponseField name="data" type="array">
      ...
      <Expandable title="Элемент массива">
        ...
        <ResponseField name="last_frame_url" type="string">
          URL изображения последнего кадра. Возвращается только для задач с `return_last_frame: true`, если модель предоставила кадр.
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
<ResponseField name="usage" type="object">
  Использование инструментов модели. Для Seedance 2 с `tools: [{ "type": "web_search" }]` поле `usage.tool_usage.web_search` содержит фактическое число поисков; `0` означает отсутствие поиска.
</ResponseField>
...
|---|---|---|---|
| `queued` | Задача отправлена, ожидает в очереди | ❌ | Подождите 5-10 секунд и повторите запрос |
| `in_progress` | Задача обрабатывается | ❌ | Подождите 10-15 секунд и повторите запрос |
| `completed` | Задача успешно завершена | ✅ | Получите видео из `result.data[0].url` |
...
<Note>
  ...
  1. **Используйте ToAPIs Task Webhook**: [Webhook](/docs/ru/api-reference/webhooks/task-webhooks) — основной канал, polling — резерв
  2. **Интервал polling**: не менее 5–10 секунд с jitter; при 429 соблюдайте `Retry-After` и exponential backoff
...
См. [лимиты асинхронных задач](/docs/ru/api-reference/rate-limits/async-tasks). Batch-запрос принимает максимум 100 task ID.
