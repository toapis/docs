---
title: "Получить статус задачи изображения"
sidebarTitle: "Получить статус изображения"
api: "GET https://toapis.com/v1/images/generations/{task_id}"
description: "Запрос статуса и результатов задачи генерации изображений"
---
...
|---|---|---|---|
| `submitted` | Задача отправлена, ожидает обработки | ❌ | Подождите не менее 5–10 секунд с jitter |
| `in_progress` | Задача обрабатывается | ❌ | Подождите не менее 5–10 секунд с jitter |
...
Интервал опроса: не менее 5–10 секунд с random jitter
...
<Note>Используйте единый [Task Webhook](/docs/ru/api-reference/webhooks/task-webhooks), polling оставьте резервом. Интервал не менее 5–10 секунд с jitter; при `429` соблюдайте `Retry-After`. Batch — максимум 100 задач. См. [лимиты](/docs/ru/api-reference/rate-limits/async-tasks).</Note>
