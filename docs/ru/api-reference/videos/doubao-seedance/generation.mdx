...
description: "Генерация видео с помощью модели ByteDance Doubao SeeDance"
---
...
<RequestExample>
```bash cURL (Быстрый просмотр горизонтального видео)
curl --request POST \
...
```bash cURL (Высококачественное вертикальное короткое видео)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
...
```bash cURL (Эффект динамического перехода - Первый/Последний кадр)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer <token>' \
  --header 'Content-Type: application/json' \
