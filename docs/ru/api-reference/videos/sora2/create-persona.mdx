---
title: "Sora2 Создание персонажа"
sidebarTitle: "Создание персонажа"
api: "POST https://toapis.com/v1/videos/generations"
description: "Извлечение персонажей из видео для последующей генерации"
---
...
```bash cURL (Из существующей задачи)
curl --request POST \
  --url https://toapis.com/v1/videos/generations \
  --header 'Authorization: Bearer YOUR_API_KEY' \
  --header 'Content-Type: application/json' \
...
```json 200 (Завершено - результат запроса)
{
...
<Steps>
  ...
  <Step title="Запрос статуса задачи">
    Используйте [Запрос статуса видео-задачи](../../tasks/video-status) для отслеживания прогресса
  </Step>
