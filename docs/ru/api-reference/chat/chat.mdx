---
title: "Чат-завершения"
sidebarTitle: "Chat Completions API"
api: "POST https://toapis.com/v1/chat/completions"
description: "Эндпоинт чат-завершений, совместимый с OpenAI, поддерживает все текстовые модели"
---
...
<ParamField body="max_tokens" type="integer">
  ...
  Если параметр не задан, используется лимит модели. Для GPT-5 шлюз преобразует это совместимое поле в `max_completion_tokens`.
</ParamField>

<ParamField body="max_completion_tokens" type="integer">
  Рекомендуемый параметр максимального числа выходных токенов для GPT-5.
</ParamField>

<ParamField body="reasoning_effort" type="string">
  Уровень рассуждения. GPT-5.6 поддерживает `none`, `low`, `medium`, `high`, `xhigh`, `max`; значение по умолчанию — `medium`.
</ParamField>
...
<Warning>
Запросы GPT-5 игнорируют `temperature` и `top_p`. Для GPT-5.6 управляйте результатом через `reasoning_effort`, промпт и явный формат ответа.
</Warning>
