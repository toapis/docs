...
description: "Общайтесь с моделями Claude в нативном формате Anthropic Messages API"
---
...
<Note>
  Если вы уже используете OpenAI SDK, рекомендуем [эндпоинт в формате OpenAI](./chat).
...
```python Python (потоковая передача)
import anthropic
