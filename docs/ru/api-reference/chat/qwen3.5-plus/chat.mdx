---
title: "Чат Qwen3.5 Plus"
sidebarTitle: "qwen3.5-plus"
api: "POST https://toapis.com/v1/chat/completions"
description: "Сбалансированная флагманская модель Alibaba с длинным контекстным окном до 256K токенов"
---

- Совместимый с OpenAI API чат-завершений
- Выберите модель `qwen3.5-plus` через параметр `model`
- Поддержка потоковой передачи (SSE)
- Поддержка многоходовых диалогов и системных промптов
- Сверхдлинное контекстное окно — до 256K входящих токенов
...
curl --request POST \
  ...
  --data '{
    ...
    "messages": [
      {
        "role": "user",
        "content": "Напиши структуру документа с требованиями к продукту"
      }
    ]
  }'
```
...
curl --request POST \
  ...
  --data '{
    ...
    "messages": [
      {
        "role": "system",
        "content": "Ты опытный продуктовый менеджер"
      },
      {
        "role": "user",
        "content": "Напиши структуру документа с требованиями к продукту"
...
response = client.chat.completions.create(
    model="qwen3.5-plus",
    messages=[
        {"role": "user", "content": "Напиши структуру документа с требованиями к продукту"}
    ]
)
...
stream = client.chat.completions.create(
    model="qwen3.5-plus",
    messages=[
        {"role": "system", "content": "Ты опытный продуктовый менеджер"},
        {"role": "user", "content": "Напиши структуру документа с требованиями к продукту"}
    ],
    stream=True
)
...
const response = await client.chat.completions.create({
  model: "qwen3.5-plus",
  messages: [{ role: "user", content: "Напиши структуру документа с требованиями к продукту" }],
});
