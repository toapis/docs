...
sidebarTitle: "Создать токен"
api: "POST https://toapis.com/v1/tokens"
description: "Создание нового API токена через API"
---
...
<ParamField body="remain_credits" type="float" default="0">
  Оставшиеся credits токена. Если передано, это поле переопределяет `remain_quota`

  Конвертация: `200` credits = $1 USD = `500000` внутренних единиц квоты

  Пример: `200` (эквивалент $1)
</ParamField>

<ParamField body="daily_credits" type="float" default="0">
  Дневной лимит токена в credits. Если передано, автоматически конвертируется в `daily_quota`

  Пример: `100` (эквивалент дневного лимита $0.5)
</ParamField>

<ParamField body="monthly_credits" type="float" default="0">
  Месячный лимит токена в credits. Если передано, автоматически конвертируется в `monthly_quota`

  Пример: `1000` (эквивалент месячного лимита $5)
</ParamField>
...
<ResponseField name="data" type="object">
  ...
  <Expandable title="Детали поля data">
    ...
    <ResponseField name="remain_credits" type="float">
      Оставшиеся credits, рассчитанные по курсу `1 USD = 200 credits`
    </ResponseField>
    ...
    <ResponseField name="daily_credits" type="float">
      Дневной лимит в credits
    </ResponseField>
    <ResponseField name="monthly_credits" type="float">
      Месячный лимит в credits
    </ResponseField>
...
```json 401 - Ошибка аутентификации
{
  "error": {
    "code": 401,
...
```json 429 - Превышен лимит запросов
{
  "error": {
    "code": 429,
