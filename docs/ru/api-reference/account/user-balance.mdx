...
sidebarTitle: "Баланс пользователя"
api: "GET https://toapis.com/v1/user/balance"
description: "Запрос остатка и использованной квоты аккаунта пользователя"
---
...
Получение остатка и использованного баланса аккаунта пользователя. Этот эндпоинт возвращает информацию о балансе на уровне пользователя, независимо от конкретных токенов. Также возвращаются поля кредитов с фиксированной конвертацией `1 USD = 200 credits`.
...
def get_user_balance():
    ...
    if data.get('success'):
        if data.get('unlimited_quota'):
            ...
            print(f"Остаток credits: {data['remain_credits']}")
...
async function getUserBalance() {
  ...
  if (data.success) {
    if (data.unlimited_quota) {
      ...
      console.log(`Остаток credits: ${data.remain_credits}`);
    }
...
```json 200 - Безлимитный пользователь
{
  "success": true,
  "remain_balance": -1,
  "used_balance": 25.5,
...
<ResponseField name="remain_credits" type="float">
  Оставшиеся credits пользователя (возвращается при успехе). Рассчитывается по курсу `1 USD = 200 credits`
</ResponseField>

<ResponseField name="used_credits" type="float">
  Использованные credits пользователя (возвращается при успехе)
</ResponseField>
